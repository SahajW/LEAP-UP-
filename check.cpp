...
//  const unsigned windowWidth = static_cast<int>(desktopMode.size.x / 2.65f);
//  const unsigned int windowHeight = static_cast<int>(desktopMode.size.y / 1.142857143f);
  const unsigned int windowWidth=1000;
  const unsigned int windowHeight=1200;
// Create a resizable window centered on the screen
sf::RenderWindow window(sf::VideoMode({windowWidth, windowHeight}), "Leap UP", sf::Style::Default);

    sf::Texture background, start, credits, quit, volume, mute,manual,start2,manual2,credits2,exit2,menu;

    if (!background.loadFromFile("bg1.png") ||
        !start.loadFromFile("play.png") ||
        !credits.loadFromFile("credits.png") ||
        !quit.loadFromFile("quit.png") ||
        !volume.loadFromFile("volume.png") ||
        !mute.loadFromFile("mute.png")||
        !start2.loadFromFile("play2.png")||
        !manual.loadFromFile("manual.png")||
        !manual2.loadFromFile("manual2.png")||
        !credits2.loadFromFile("credits2.png")||
        !exit2.loadFromFile("exit2.png")||
